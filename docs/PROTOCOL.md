...
| `GET /api/health` | `?k=KEY` | the debugging question "is the lobby pairing people at all?". For everyone: `{ ok, paired }`, one bit, served from the edge. With the operator's key (the `HEALTH_KEY` secret): counts of registrations, opens, rooms, and reports since the lobby first counted, plus people, people who ran Claude, on now, waiting now, rooms now, blocked now. Counts only; there is no per-person record behind them. Refreshed at most every five seconds, with the sweep, so it is never stale (D-101, D-102) |
