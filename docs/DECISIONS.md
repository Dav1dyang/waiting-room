...
**D-101  A health check, not a dashboard** (accepted; David, after the Reddit post, 2026-09-08)
After the post, David wanted to know one thing: is this actually working, are people being paired? He was explicit that he does not want to invade anyone's privacy. The lobby records nothing about people, and the one trace of a pairing (`lastPeers`) is pruned after the sixty-second cooldown, so the question was unanswerable, which is the wrong kind of privacy: a system nobody can tell is working. The answer is a health check, `GET /api/health`: counts of registrations, window opens, rooms formed, and reports since the lobby first counted, plus what is true right now (people, people who ran Claude with it on, on now, waiting now, rooms now, blocked now). Numbers only. There is no per-person record behind them to read, not even for the operator: tokens are random, no address is stored (only the salted hash for the report rule), and who met whom is never written down. The route needs no token because a count of strangers is not a record of any of them. Cloudflare's own traffic analytics would have said more, but the CLI login has no analytics scope, and a traffic number was not the question.

**D-102  One bit for everyone, the counts for the operator** (accepted; Codex review of D-101, 2026-09-09)
Codex on the health check: exact live counts on a public route, polled every five seconds, are a signal in a small lobby (the moment a room forms, or a report is filed, is visible to anyone watching), and every poll woke the one lobby object, which is what the free plan meters. So the route is split. Everyone gets `{ ok, paired }`, served from the edge and remembered per isolate for five seconds. The counts need the operator's key, a secret on the Worker that never leaves David's `.dev.vars`. On refresh the lobby advances its clock first, so a room that is over is not counted as open. And every `/api` route but `hook` meets a per-address limit at the edge (120 a minute) before the lobby is woken; hooks carry a token and their own caps. Also from the review, no issue found: the unknown-token short cuts return the same shapes the core would; counters bump only on real events; old snapshots hydrate with zeroed counters.
