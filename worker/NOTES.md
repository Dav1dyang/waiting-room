...
- The whole lobby is one storage value. At roughly 300 bytes a token that is about 6,000 registrations before the 2 MB per-value ceiling; the fix is rows per token and per room. Since D-99 a ceiling of 3,000 tokens (MAX_TOKENS) with a sweep of register-only tokens keeps the value under half that; a flood from many homes can still make registration say busy for newcomers for up to an hour. Probes are capped at twenty per token, 2 KB each.
...
- Every cap keys on the client address (IPv6 by /64). A shared address is the only identity the lobby has. Since D-102 an edge rate limit (120 a minute per address) sits in front of every `/api` route but `hook`; Turnstile in front of `/api/register` remains the Phase 2 answer for a flood from many addresses. The call itself is peer to peer: browsers exchange addresses as every WebRTC call does; hiding that means relay-only ICE through TURN, a cost decision (D-82).
- The invite-guess throttle has a global cap (three hundred failures an hour): anyone who knows the lobby URL can spend it and hold registration shut for an hour. The URL is not public in the alpha; a per-address-only rule, or Turnstile in front of `/api/register`, is the Phase 2 answer.
