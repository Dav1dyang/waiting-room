...
- Every cap keys on the client address (IPv6 by /64). A shared address is the only identity the lobby has; Turnstile in front of `/api/register`, or edge rate limiting, is the Phase 2 answer. The call itself is peer to peer: browsers exchange addresses as every WebRTC call does; hiding that means relay-only ICE through TURN, a cost decision (D-82).
